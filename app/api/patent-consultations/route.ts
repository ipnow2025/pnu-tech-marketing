...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const sql = `
      UPDATE pnu_techfair_patent_consultations 
    ...
    if (result.affectedRows === 0) {
      return NextResponse.json(
        { error: '해당 특허 상담신청을 찾을 수 없거나 이미 삭제되었습니다.' },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: '특허 상담신청이 성공적으로 삭제되었습니다.'
    });
    
  } catch (error) {
    console.error('특허 상담신청 삭제 오류:', error);
    return NextResponse.json(
      { error: '특허 상담신청 삭제 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
