...
export async function POST(request: NextRequest) {
  try {
    ...
    // 필수 필드 검증
    if (!companyName || !ceoName || !contactName || !mobilePhone || !email || !desiredTechnology || !technologyRequirements) {
    ...
    // SQL 쿼리 작성
    const sql = `
      INSERT INTO pnu_techfair_technology_consultations (
        company_name,
        ceo_name,
        establishment_date,
        company_phone,
    ...
    return NextResponse.json({
      success: true,
      message: '기술상담신청이 성공적으로 저장되었습니다.',
      data: { id: result.insertId }
    });

  } catch (error) {
    console.error('기술상담신청 저장 오류:', error);
    return NextResponse.json(
      { error: '기술상담신청 저장 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  try {
    const sql = `
      SELECT * FROM pnu_techfair_technology_consultations 
    ...
    console.error('기술상담신청 조회 오류:', error);
    return NextResponse.json(
      { error: '기술상담신청 조회 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
